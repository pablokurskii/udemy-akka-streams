...
object GraphBasics extends App {
  ...
  val inputTwo = Source(1 to 10)

  val sinkOne = Sink.foreach[Int](x => println(s"1st sink $x"))
  val sinkTwo = Sink.foreach[Int](x => println(s"2nd sink $x"))
  ...
  graphTwoSinks.run() // run the graph and materialize it
